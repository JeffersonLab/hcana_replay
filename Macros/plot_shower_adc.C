#include <TH1.h>
#include <TH2.h>
#include <TCanvas.h>
#include <TStyle.h>
#include <TROOT.h>
#include <TStyle.h>
#include <TLegend.h>
#include <TMath.h>
#include <TBox.h>
#include <TPolyLine.h>
void plot_shower_adc(Int_t nrun) {
  TStyle *MyStyle = new TStyle("MyStyle"," My Style");
 MyStyle->SetOptStat(10);
 MyStyle->SetTitleOffset(1.,"Y");
 MyStyle->SetTitleOffset(.7,"X");
 MyStyle->SetLabelSize(0.04,"XY");
 MyStyle->SetTitleSize(0.06,"XY");
 MyStyle->SetPadLeftMargin(0.12);
 MyStyle->SetStatFontSize(0.06);
 gROOT->SetStyle("MyStyle");
TCanvas *shadc = new TCanvas("shadc","Shower Compare ADC hit spectra ",1200,1000);
 TFile *fh1 =  new TFile(Form("paw/%05d.root",nrun));
  TFile *fh2 =  new TFile(Form("Rootfiles/compare_hodo_%05d.root",nrun));
  //
  TString hname1[6] = { "h213","h217","h214","h218","h215","h216"};
  TString hname2[6] = { "chposadc1","chnegadc1","chposadc2","chnegadc2","chposadc3","chposadc4"};
  TString dname[6] = { "Diff A+ ADC hits","Diff A- ADC hits","Diff B+ ","Diff B-","Diff C+","Diff D+"};
  Int_t l=0;
  shadc->Divide(2,6);
  for ( int i=0 ; i < 6 ; i++) {
    l++;
    shadc->cd(l);
    ((TH1F*)fh1->Get(hname1[i]))->Draw();
    ((TH1F*)fh2->Get(hname2[i]))->Draw("same");
    ((TH1F*)fh2->Get(hname2[i]))->SetLineColor(2);
    l++;
    shadc->cd(l);
    TH1F* diff2=((TH1F*)fh1->Get(hname1[i]))->Clone() ;
    diff2->SetLineColor(3);
    diff2->SetTitle(dname[i]);
    diff2->Add(((TH1F*)fh1->Get(hname1[i])),((TH1F*)fh2->Get(hname2[i])),1,-1);
    diff2->Draw();
  }
  //
}
