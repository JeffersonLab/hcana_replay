...
char *s = new char[1];
  ...
  for (Long64_t ni=0 ; ni<nent_loop ;ni++) {
      ...
      hyfp_hcana[i]->Fill(myhcana->S_dc_y[i]);
      hxpfp_hcana[i]->Fill(myhcana->S_dc_xp[i]*1000.);
      hypfp_hcana[i]->Fill(myhcana->S_dc_yp[i]*1000.);
      hxfp_eng[i]->Fill(myengine->sdc_xfp[i]);
      hyfp_eng[i]->Fill(myengine->sdc_yfp[i]);
      hxpfp_eng[i]->Fill(myengine->sdc_xpfp[i]*1000);
      hypfp_eng[i]->Fill(myengine->sdc_ypfp[i]*1000);
      hchi2_hcana[i]->Fill(myhcana->S_tr_chi2[i]);
      hchi2_eng[i]->Fill(myengine->sdc_chi2[i]);
      hytg_hcana[i]->Fill(myhcana->S_tr_tg_y[i]);
      hxptg_hcana[i]->Fill(myhcana->S_tr_tg_th[i]*1000.);
      hyptg_hcana[i]->Fill(myhcana->S_tr_tg_ph[i]*1000.);
      hdelta_hcana[i]->Fill(myhcana->S_tr_tg_dp[i]);
      hytg_eng[i]->Fill(myengine->sdc_ytg[i]);
      hxptg_eng[i]->Fill(myengine->sdc_xptg[i]*1000);
      hyptg_eng[i]->Fill(myengine->sdc_yptg[i]*1000);
      hdelta_eng[i]->Fill(myengine->sdc_delta[i]);
	hchi2_sub[i]->Fill((myhcana->S_tr_chi2[i]-myengine->sdc_chi2[i]));
	hxfp_sub[i]->Fill((myhcana->S_dc_x[i]-myengine->sdc_xfp[i]));
	hyfp_sub[i]->Fill((myhcana->S_dc_y[i]-myengine->sdc_yfp[i]));
	hypfp_sub[i]->Fill((myhcana->S_dc_yp[i]-myengine->sdc_ypfp[i]));
        hxpfp_sub[i]->Fill((myhcana->S_dc_xp[i]-myengine->sdc_xpfp[i]));
	hytg_sub[i]->Fill((myhcana->S_tr_tg_y[i]-myengine->sdc_ytg[i]));
	hyptg_sub[i]->Fill((myhcana->S_tr_tg_ph[i]-myengine->sdc_yptg[i])*1000);
         hxptg_sub[i]->Fill((myhcana->S_tr_tg_th[i]-myengine->sdc_xptg[i])*1000);
         hdelta_sub[i]->Fill((myhcana->S_tr_tg_dp[i]-myengine->sdc_delta[i]));
       if ( TMath::Abs(myhcana->S_tr_chi2[i]-myengine->sdc_chi2[i]) >= 10. ) hchi2_eng2[i]->Fill(myengine->sdc_chi2[i]);
      }   
    }
  }
 ...
 Double_t xlo_pass_xp[3][4],xlo_pass_yp[3][4];
  Int_t binlo[4]={1,901,991,1000},binhi[4]={2001,1101,1011,1001};
   for (i=0 ; i<3 ;i++) {
     ...
     if ( good_events[i] == 0) good_events[i]=-1.;
