...
#ifndef analyze_engine_hms_tree_h
#define analyze_engine_hms_tree_h
...
public :
   ...
   analyze_engine_hms_tree(TString ifile,TTree *tree=0);
   virtual ~analyze_engine_hms_tree();
...
#ifdef analyze_engine_hms_tree_cxx
analyze_engine_hms_tree::analyze_engine_hms_tree(TString ifile,TTree *tree) : fChain(0) 
{
...
analyze_engine_hms_tree::~analyze_engine_hms_tree()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t analyze_engine_hms_tree::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t analyze_engine_hms_tree::LoadTree(Long64_t entry)
{
...
void analyze_engine_hms_tree::Init(TTree *tree)
{
...
Bool_t analyze_engine_hms_tree::Notify()
{
...
void analyze_engine_hms_tree::Show(Long64_t entry)
{
...
Int_t analyze_engine_hms_tree::Cut(Long64_t entry)
{
...
void analyze_engine_hms_tree::PrintTrack(Long64_t entry)
{
// Print contents of entry.
// If entry is not specified, print current entry
   if (!fChain) return;
   ...
   cout << "HMS  Engine event number = " << evnum << " event type = " << evtype << endl;
  cout << " Number of Tracks  = " << hdc_ntr << endl;
   ...
   cout << " SOS Engine event number = " << evnum << " event type = " << evtype << endl;
...
#endif // #ifdef analyze_engine_hms_tree_cxx
