...
#include <iomanip>
#include <TStyle.h>
void scan_comp( TString hcana_file, TString engine_file) {
  ...
  hxpfp_sub[i] = new TH1F(Form("hxpfp_sub%d",i),"; Xpfp (HCANA-ENGINE); Counts",100,-.0001,.0001);
  //
  ...
  hypfp_sub[i] = new TH1F(Form("hypfp_sub%d",i),"; Ypfp  (HCANA-ENGINE); Counts",100,-.0001,.0001);
  //
  ...
  hxfp_sub[i] = new TH1F(Form("hxfp_sub%d",i),"; Xfp (HCANA-ENGINE); Counts",100,-.001,.001);
  //
  ...
  hyfp_sub[i] = new TH1F(Form("hyfp_sub%d",i),"; Yfp (HCANA-ENGINE); Counts",60,-.001,.001);
  //
  ...
  hyptg_sub[i] = new TH1F(Form("hyptg_sub%d",i),"; Yptg (mr) (HCANA-ENGINE); Counts",100,-.1,.1);
  //
  ...
  hytg_sub[i] = new TH1F(Form("hytg_sub%d",i),"; Ytg (cm) (HCANA-ENGINE); Counts",60,-.01,.01);
  //
...
char *s = new char[1];
  for (Long64_t ni=0 ; ni<nent_hcana ;ni++) {
      ...
      if ((myhcana->H_dc_ntrack) != (myengine->dc_ntr) )hntr_2d->Fill(myhcana->H_dc_ntrack,myengine->dc_ntr);
  if ( myengine->dc_ntr==myhcana->H_dc_ntrack && myengine->dc_ntr <= 3 ) {
    for (i=0 ; i<myengine->dc_ntr ;i++) {
      ...
      hchi2_eng[i]->Fill(myengine->dc_chi2[i]);
      ...
      hytg_eng[i]->Fill(myengine->dc_ytg[i]);
      hxptg_eng[i]->Fill(myengine->dc_xptg[i]*1000);
      hyptg_eng[i]->Fill(myengine->dc_yptg[i]*1000);
      hdelta_eng[i]->Fill(myengine->dc_delta[i]);
	hchi2_sub[i]->Fill((myhcana->H_tr_chi2[i]-myengine->dc_chi2[i]));
	hxfp_sub[i]->Fill((myhcana->H_dc_x[i]-myengine->dc_xfp[i]));
	hyfp_sub[i]->Fill((myhcana->H_dc_y[i]-myengine->dc_yfp[i]));
	hypfp_sub[i]->Fill((myhcana->H_dc_yp[i]-myengine->dc_ypfp[i]));
        hxpfp_sub[i]->Fill((myhcana->H_dc_xp[i]-myengine->dc_xpfp[i]));
	hytg_sub[i]->Fill((myhcana->H_tr_tg_y[i]-myengine->dc_ytg[i]));
	hyptg_sub[i]->Fill((myhcana->H_tr_tg_ph[i]-myengine->dc_yptg[i])*1000);
         hxptg_sub[i]->Fill((myhcana->H_tr_tg_th[i]-myengine->dc_xptg[i])*1000);
         hdelta_sub[i]->Fill((myhcana->H_tr_tg_dp[i]-myengine->dc_delta[i]));
